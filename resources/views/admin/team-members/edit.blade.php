...
@section('title', 'Modifica: ' . $teamMember->name)
...
@section('content')
    ...
    <div class="bg-white rounded-xl border border-gray-200/60 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100">
            <h2 class="text-sm font-heading font-semibold text-gray-900">Modifica membro del team</h2>
        </div>
        <form method="POST" action="{{ route('admin.team-members.update', $teamMember) }}" enctype="multipart/form-data" class="p-5">
            @csrf
            @method('PUT')
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="bio" id="bio" rows="4"
                        class="w-full rounded-lg border-gray-300 text-sm focus:border-primary focus:ring-primary resize-none @error('bio') border-red-300 @enderror">{{ old('bio', $teamMember->bio) }}</textarea>
                    @error('bio')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="experience_summary" id="experience_summary" rows="4"
                        class="w-full rounded-lg border-gray-300 text-sm focus:border-primary focus:ring-primary resize-none tinymce-editor @error('experience_summary') border-red-300 @enderror">{{ old('experience_summary', $teamMember->experience_summary) }}</textarea>
                ...
                <div>
                    ...
                    @if($teamMember->photo)
                        <div class="mb-3 flex items-center gap-3">
                            <img src="{{ asset($teamMember->photo) }}" alt="{{ $teamMember->name }}" class="w-16 h-16 rounded-full object-cover">
                            <span class="text-xs text-gray-400">Foto attuale</span>
                        </div>
                    @endif
                    ...
                    <p class="text-xs text-gray-400 mt-1">Lascia vuoto per mantenere la foto attuale</p>
                    @error('photo')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="url" name="linkedin_url" id="linkedin_url" value="{{ old('linkedin_url', $teamMember->linkedin_url) }}" placeholder="https://linkedin.com/in/..."
