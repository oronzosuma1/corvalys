...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <p class="text-sm text-gray-500">Gestisci le pagine del sito</p>
        <a href="{{ route('admin.pages.create') }}" class="inline-flex items-center px-4 py-2 bg-primary-dark text-white text-sm font-medium rounded-lg hover:bg-primary transition-colors">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($pages ?? [] as $page)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-5 py-3 text-gray-600">
                                <code class="text-xs bg-gray-100 px-2 py-0.5 rounded">/{{ $page->slug }}</code>
                            </td>
                            <td class="px-5 py-3">
                                <form method="POST" action="{{ route('admin.pages.toggle', $page) }}" class="inline">
                                    ...
                                    <button type="submit" class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium transition-colors
                                        ...
                                        <span class="w-1.5 h-1.5 rounded-full mr-1.5 {{ $page->is_published ? 'bg-green-500' : 'bg-gray-400' }}"></span>
                                        {{ $page->is_published ? 'Pubblicata' : 'Bozza' }}
                                    </button>
                                </form>
                            </td>
                            <td class="px-5 py-3 text-xs text-gray-400">{{ $page->created_at->format('d/m/Y') }}</td>
                            <td class="px-5 py-3">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('admin.pages.edit', $page) }}" class="inline-flex items-center text-primary hover:text-primary-dark text-xs font-medium">
                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                        Modifica
                                    </a>
                                    <form method="POST" action="{{ route('admin.pages.destroy', $page) }}" class="inline" onsubmit="return confirm('Sei sicuro di voler eliminare questa pagina?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="5" class="px-5 py-8 text-center text-gray-400 text-sm">Nessuna pagina presente</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
